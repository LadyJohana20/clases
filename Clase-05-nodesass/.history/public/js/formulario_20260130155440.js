...
formProductos.addEventListener("submit", (e) => {
    ...
    //enviarAjax(producto);// funcion 
    enviarFetch(producto);// funcion fetch API
...
/*function enviarDatosAlBackend(datos, lugar) { ejemplo de funcion generica
    ...
    function enviarAjax(producto) {
        //creamos instancia de hxmlhttprequest
        let xhr = new XMLHttpRequest();
...
//configuramos los hendlers
 ...
 //http no envia datos de objetos literales, se envian en formato string
...
//funcion fetch API para enviar datos
const enviarFetch = (producto) => {
    //obtener la url del backend
    let url = "http://localhost:3000/reciboproductos";
    // enviamos la peticion usando fetch
    fetch(url, {
        method: "POST",
        headers: {      
